...
public interface PublishHistoryMapper {
     ...
     */
    @Insert("insert into tb_publish_history(id,picture_url,title,type,download_num,attachment_url,table_choose,delete_flag) values(#{id},#{pictureUrl},#{title},#{type},#{downloadNum},#{attachmentUrl},#{tableChoose},#{deleteFlag})")
     ...
     * @return
     */
    @Select("select id,picture_url as pictureUrl,title,download_num as downloadNum,attachment_url as attachmentUrl,type from tb_publish_history where delete_flag = 0 limit #{param1},#{param2}")
